#!/bin/bash
# ============================================================
#  moOde-radio-plus — Install Script (V7.11 SAFE FALLBACK)
...
if grep -q "moode-sse-patch" "$JS_TARGET" 2>/dev/null; then
    ...
    if [ ! -f "${JS_TARGET}.bak.original" ]; then
        sudo cp "$JS_TARGET" "${JS_TARGET}.bak.original"
    fi
...
echo ""
echo "============================================="
echo "  Installazione FALLBACK completata."
echo ""
